function [HyRED, U_matrix, U_rhs] = CSM_HyperReduction_offline(FOM, S_rhs, S_matrix, tolPOD)
...
[IDEIM_rhs_elem, ~, IDEIM_rhs_bound]    = CSM_DEIM_Index_to_Elements('rhs',    HyRED.IDEIM_rhs, ndf, node_to_element, ...
    node_to_boundary, FOM.MESH.internal_dof, FOM.MESH.numNodes, FOM.MESH.dim);
...
[HyRED.MESH.jac, HyRED.MESH.invjac, HyRED.MESH.h] = geotrasf(HyRED.MESH.dim, HyRED.MESH.vertices, HyRED.MESH.elements);   

for k = 1 : FOM.MESH.dim
    HyRED.MESH.Neumann_side{k} = intersect(HyRED.MESH.Neumann_side{k}, IDEIM_all_bound);
end
